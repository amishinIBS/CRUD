...
public class ReflectionUtil {
    ...
    public static Map<String, Class> getEntityFieldMap(Class<?> entityClass, Class<?> clazz, Type genericSuperclass) {
        Map<String, Class> entityFieldMap = Arrays.stream(clazz.getDeclaredFields()).collect(Collectors.toMap(Field::getName, f -> {
            if (f.getGenericType() instanceof TypeVariable && genericSuperclass != null) {
                TypeVariable<? extends Class<?>>[] typeParameters = clazz.getTypeParameters();
                int index = typeIndex(typeParameters, f);
                if(index >= 0) {
                    return ReflectionUtil.getGenericParameterClass(entityClass, clazz, index);
                } else {
                    return f.getType();
                }
            } else {
        ...
        if (clazz.getSuperclass() != null) {
            getEntityFieldMap(entityClass, clazz.getSuperclass(), clazz.getGenericSuperclass()).entrySet().stream()
                    .forEach(f -> entityFieldMap.putIfAbsent(f.getKey(), f.getValue()));
        }
        return entityFieldMap;
    }

    private static int typeIndex(TypeVariable<? extends Class<?>>[] typeParameters, Field f) {
        int i = 0;
        boolean found = false;
        while (i < typeParameters.length && !found) {
            found = f.getGenericType() == typeParameters[i];
            if(!found) {
                i++;
            }
        }
